...
const registerSubject = catchAsyn(async(req,res,next)=>{
    ...
    if(duplicate) return res.status(409).json({status:"success", message:"Duplicate , code number must be unique"}); //Conflict
...
const getSubjectById = catchAsyn(async(req,res,next)=>{
    ...
    res.status(200).json({status:'success',result:getSubject.length, data:getSubject})
})
const updateSubject = catchAsyn(async(req,res,next)=>{
    ...
    let query = {subjectCode:subjectCode,subjectName:subjectName,subjectShortForm:subjectShortForm}
    const result = await Subject.updateOne({_id:_id},query,{upsert:true})
